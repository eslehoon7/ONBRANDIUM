...
const ContactForm: React.FC<ContactFormProps> = ({ config }) => {
  ...
  const N8N_WEBHOOK_URL = 'https://eslehoon.app.n8n.cloud/webhook/onbrandium-inquiry';
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input type="text" value={name} onChange={handleNameChange} required placeholder="홍길동"
            ...
            <div className="space-y-2">
              ...
              <input type="tel" value={phone} onChange={handlePhoneChange} required maxLength={13} placeholder="010-0000-0000"
                className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-4 text-white placeholder:text-white/20 focus:border-cyan-400 outline-none transition-all" />
            </div>
          </div>
          <div className="space-y-2">
            <label className="text-[10px] uppercase font-bold tracking-widest text-white/40 block ml-1">이메일</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required placeholder="example@onbrandium.com"
              className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-4 text-white placeholder:text-white/20 focus:border-cyan-400 outline-none transition-all" />
          </div>
          <div className="space-y-2">
            ...
            <textarea rows={5} value={message} onChange={(e) => setMessage(e.target.value)} required
              ...
              className="w-full bg-black/50 border border-white/10 rounded-xl px-4 py-4 text-white placeholder:text-white/20 focus:border-cyan-400 outline-none transition-all resize-none"></textarea>
          </div>
          <button type="submit" disabled={isSubmitting || isSuccess}
            className={`w-full py-5 rounded-xl font-bold transition-all shadow-lg flex items-center justify-center gap-2 ${isSuccess ? 'bg-[#1DB978] text-white cursor-default text-lg' : 'hover:scale-[1.02] active:scale-[0.98] text-lg font-black'}`}
            style={!isSuccess ? { backgroundColor: config.accentColor, color: '#000' } : { boxShadow: '0 10px 30px -10px rgba(29, 185, 120, 0.5)' }}>
            {isSubmitting ? (<><svg className="animate-spin h-5 w-5 text-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>보내는중...</>) 
            : isSuccess ? (<><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>접수 완료되었습니다. 감사합니다.</>) 
            : '문의 보내기'}
          </button>
        </form>
      </div>
      {showModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="relative w-full max-w-sm bg-neutral-900 border border-white/10 rounded-3xl shadow-2xl p-8 text-center">
            ...
            <button onClick={() => setShowModal(false)} className="w-full py-3 rounded-xl font-bold text-black bg-cyan-400 hover:bg-cyan-300 transition-all">확인</button>
          </div>
        </div>
      )}
    </>
